...
### Added
- **Nimbus Mouse Filter driver** (`driver/`): A KMDF upper filter on the mouse class, derived from Microsoft's `moufiltr` sample, that withholds physical mouse packets from `mouclass` while Nimbus holds `\\.\NimbusMouseFilter` open with isolation on and delivers them to Nimbus through `ReadFile`. Pass-through by default; isolation is cleared on handle cleanup and by a 2 s read watchdog; the keyboard is never touched. Builds and test-signs with WDK 10.0.26100; not yet loaded on hardware or attestation-signed. Dev scripts for build, test signing, install and uninstall included.
- **Windows mouse isolation client** (`src/mouse_isolation_win.py`): Same `MouseIsolation` class API as the Linux `mouse_isolation` module (evdev button codes included), so the bridge integration from the Linux branch carries over unchanged. `python -m src.mouse_isolation_win --status` reports the driver's counters.
- **Mouse filter probe** (`tests/probe_mouse_filter_windows.py`): Unattended lifecycle, handle-drop, watchdog and exclusivity checks, plus attended pass-through / isolated / released phases against the fake Raw Input game.
