...
- Linux runs from source: `vgamepad` drives `/dev/uinput` there instead of ViGEmBus, with identical axes/buttons, so `ViGEmInterface` is the only backend off Windows (`pyvjoy` is Windows-only and `sys.exit()`s at import). `window_utils`/`borderless`/`mouse_hider` import fine on Linux but every function no-ops, so gate user-facing Win32 features on `sys.platform == "win32"`, not on the `*_AVAILABLE` flag alone.
