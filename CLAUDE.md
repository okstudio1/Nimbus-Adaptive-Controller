...
- `vgamepad` (the package behind `ViGEmInterface`) ships its own Linux backend that creates the virtual pad through `uinput`/`libevdev` instead of ViGEmBus, so Xbox/Adaptive/Custom profiles already work on Linux from source, verified end-to-end (evdev, SDL2, browser Gamepad API, a live Steam/Proton game). `pyvjoy` has no Linux equivalent, so `VJoyInterface` stays in permanent simulation mode there — flight-sim profiles have no real output on Linux until a Linux vJoy replacement lands. `pyvjoy` also raises `SystemExit` (not `ImportError`) when its DLL won't load, so its import guard must catch `SystemExit` too, not just `Exception`/`ImportError`. See [`docs/setup/INSTALLATION.md`](docs/setup/INSTALLATION.md#linux-installation) for setup and known gaps (Borderless Gaming/Game Focus Mode are correctly gated to Win32; the Game Mode ribbon button is not yet gated and is a dead button on Linux).
