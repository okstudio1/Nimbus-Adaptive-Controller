...
**On Linux** no driver is needed: the Xbox pad is a kernel `uinput` device, and the same Game Mode button runs controller-mode enforcement plus an X11 Game Focus Mode equivalent. Borderless conversion and cursor release are Windows-only. See [Linux Setup](setup/LINUX.md).
