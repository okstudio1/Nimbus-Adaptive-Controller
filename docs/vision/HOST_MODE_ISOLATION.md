...
| `vigem_interface.py` (`src/padbus_client.py`, requires ViGEmBus) | A uinput pad with the same `X360Pad` method names. **Verified end to end**, not just architecturally true, see the update note above. vgamepad's Linux backend (evdev/uinput) is the reference for it, but since 2026-09-09 Nimbus no longer depends on vgamepad on Windows, so this shim is written for the port rather than inherited. |
...
2. **vgamepad's Linux backend is marked experimental upstream, but the part Nimbus needs works.** Buttons and axes, everything `ViGEmInterface` calls, are confirmed correct end to end (see the update note above). Rumble and LEDs are Windows-only and not ported, and Nimbus never calls them, so that gap costs nothing today.
