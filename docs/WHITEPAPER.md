...
The consequence is architectural: for that tier Nimbus must own the mouse below `win32k`. On Linux that is one `EVIOCGRAB` ioctl (the `linux-uinput-support` branch). On Windows it is a small KMDF upper filter on the mouse class, the Nimbus Mouse Filter in `driver/`, which passes everything through until Nimbus asks for isolation and then hands the packets to Nimbus instead of `mouclass`. Nimbus draws its own cursor while isolated, exactly as on Linux. The driver builds and is test-signed; it is not yet validated on hardware or attestation-signed, and it is not part of any release. Design and status: `docs/vision/WINDOWS_MOUSE_FILTER_PLAN.md`.
